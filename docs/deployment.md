...
API/restore 持锁期间每 10 秒刷新锁文件的租约心跳。有效新版锁连续 60 秒没有心跳且本机持锁 PID 不存活时，新进程会自动将它归档为 `.runtime-lock.stale-*` 后接管；不含租约字段的旧版锁使用 10 分钟的保守过期时间。因此 Docker kill、OOM 或宿主机重启留下的有效僵尸锁会自动恢复，而正在心跳的新版跨容器活锁不会被误删。

只有在人工确认 API/restore 进程已经异常退出后，才可设置 `SUBMAIL_BREAK_STALE_RUNTIME_LOCK=YES` 接管内容无法校验或尚未过期的外部锁。系统即使看到该开关，也不会删除确认仍存活的本机 PID 锁；被接管的旧锁会保留为审计工件。
